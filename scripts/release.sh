...
# Automated release script for peter-hook
...
TAG_VERSION=$(git describe --exact-match --tags HEAD | sed "s/peter-hook-v//")
if [ "$TAG_VERSION" != "$NEW_VERSION" ]; then
    log_error "Created tag version ($TAG_VERSION) doesn't match expected version ($NEW_VERSION)"
    exit 1
fi

log_success "Created tag: peter-hook-v$NEW_VERSION"

# Final confirmation
echo
log_info "Ready to push release:"
log_info "  Version: $NEW_VERSION"
log_info "  Tag: peter-hook-v$NEW_VERSION"
...
log_info "Monitor the release at: https://github.com/workhelix/peter-hook/actions"
log_info "Release will be available at: https://github.com/workhelix/peter-hook/releases/tag/peter-hook-v$NEW_VERSION"